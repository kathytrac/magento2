...
class Data extends AbstractHelper
{
	...
	private function getProductImages($product, $_config, $prefix = 'imgcfgcat')
	{
		$defaults = [
			'product_image' => 1,
	...
	public function _resizeImage($file, $config = [], $type = "product", $folder = 'resized')
	{
		...
		if(empty($file))
			$file = $this->_block->getViewFileUrl('Sm_SuperCategories::images/nophoto.jpg');
		...
		$new_image = $file;
		if (file_exists($cache_dir . '/' . $_file_name) && ($cache_dir . '/' . $_file_name)){
